...
namespace aspect {
	const char* LogLevel::ToString(LogLevel::Level level)
	{
		...
		switch (level)
		{
		case aspect::LogLevel::DEBUG:
			break;
		case aspect::LogLevel::INFO:
			break;
		case aspect::LogLevel::WARN:
			break;
		case aspect::LogLevel::ERROR:
			break;
		case aspect::LogLevel::FATAL:
			break;
		default:
			break;
		}*/
...
#undef XX
    ...
    void Logger::setFormatter(const std::string& val) {
        ...
        aspect::LogFormatter::ptr new_val(new aspect::LogFormatter(val));
        if(new_val->isError()) {
...
aspect::ConfigVar<std::set<LogDefine> >::ptr g_log_defines =
    aspect::Config::Lookup("logs", std::set<LogDefine>(), "logs config");

struct LogIniter {
    LogIniter() {
        g_log_defines->addListener(0xF1E231,[](const std::set<LogDefine>& old_value,
                    const std::set<LogDefine>& new_value){
            ASPECT_LOG_INFO(ASPECT_LOG_ROOT()) << "on_logger_conf_changed";
            for(auto& i : new_value) {
                auto it = old_value.find(i);
                aspect::Logger::ptr logger;
                if(it == old_value.end()) {
                    //新增logger
                    logger = ASPECT_LOG_NAME(i.name);
                } else {
                    if(!(i == *it)) {
                        //修改的logger
                        logger = ASPECT_LOG_NAME(i.name);
                ...
                for(auto& a : i.appenders) {
                    aspect::LogAppender::ptr ap;
                    if(a.type == 1) {
                        ap.reset(new FileLogAppender(a.file));
                    } else if(a.type == 2) {
                        if(1/*!aspect::EnvMgr::GetInstance()->has("d")*/) {
            ...
            for(auto& i : old_value) {
                ...
                if(it == new_value.end()) {
                    //删除logger
                    auto logger = ASPECT_LOG_NAME(i.name);
